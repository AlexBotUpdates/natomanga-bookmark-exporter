    ...
    async function fetchPage(page, retryCount = 0) {
        ...
        try {
            ...
            items.forEach(item => {
                ...
                // Function to find the 'a' tag after a span containing specific text
                function findLinkAfterText(item, searchText) {
                    const spans = item.querySelectorAll('span');
                    for (const span of spans) {
                        if (span.textContent.includes(searchText)) {
                            return span.querySelector('a'); // Find the 'a' tag within the span
                        }
                    }
                    return null; // Return null if not found
                }

                const lastChapterReadElement = findLinkAfterText(item, 'Viewed :');
                const lastChapterUpdatedElement = findLinkAfterText(item, 'Current :');

                if (titleElement) {
                    bookmarks.push({
                        title: titleElement.innerText.trim(),
                        lastUpdated: lastUpdatedElement ? lastUpdatedElement.innerText.trim() : "N/A",
                        lastChapterRead: lastChapterReadElement ? lastChapterReadElement ? lastChapterReadElement.innerText.trim() : "N/A" : "N/A",
                        lastChapterUpdated: lastChapterUpdatedElement ? lastChapterUpdatedElement ? lastChapterUpdatedElement.innerText.trim() : "N/A" : "N/A"
                    });
                }
            });
